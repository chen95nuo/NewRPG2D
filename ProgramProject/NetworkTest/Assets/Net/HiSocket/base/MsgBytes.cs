    ...
    public class MsgBytes : MsgBase
    {
        ...
        public T Read<T> (int length = 0) {
            ...
            if (typeof (T) == typeof (byte))
                return (T)Convert.ChangeType (_iByteArray.Read (sizeof (byte)), typeof (T));
            if (typeof (T) == typeof (byte[])) {
                if (length == 0)
                    throw new Exception ("read byte[] length:" + length);
                return (T)Convert.ChangeType (_iByteArray.Read (length), typeof (T));
            }
            if (typeof (T) == typeof (bool))
                return (T)Convert.ChangeType (_iByteArray.Read (sizeof (bool)), typeof (T));
            if (typeof (T) == typeof (char))
                return (T)Convert.ChangeType (_iByteArray.Read (sizeof (char)), typeof (T));
            if (typeof (T) == typeof (double))
                return (T)Convert.ChangeType (_iByteArray.Read (sizeof (double)), typeof (T));
            if (typeof (T) == typeof (short))
                return (T)Convert.ChangeType (_iByteArray.Read (sizeof (short)), typeof (T));
            if (typeof (T) == typeof (int))
                return (T)Convert.ChangeType (_iByteArray.Read (sizeof (int)), typeof (T));
            if (typeof (T) == typeof (long))
                return (T)Convert.ChangeType (_iByteArray.Read (sizeof (long)), typeof (T));
            if (typeof (T) == typeof (float))
                return (T)Convert.ChangeType (_iByteArray.Read (sizeof (float)), typeof (T));
            if (typeof (T) == typeof (string)) {
                //index += length;
                if (length == 0)
                    throw new Exception ("read string length:" + length);
                return (T)Convert.ChangeType (Encoding.UTF8.GetString (_iByteArray.Read (length)), typeof (T));
            }
            if (typeof (T) == typeof (ushort))
                return (T)Convert.ChangeType (_iByteArray.Read (sizeof (ushort)), typeof (T));
            if (typeof (T) == typeof (uint))
                return (T)Convert.ChangeType (_iByteArray.Read (sizeof (uint)), typeof (T));
            if (typeof (T) == typeof (ulong))
                return (T)Convert.ChangeType (_iByteArray.Read (sizeof (ulong)), typeof (T));
        ...
        public void Write<T> (T paramValue) {
            if (paramValue is byte) {
                ...
                _iByteArray.Write (new[] { tempVelue }, 1);
            }
                ...
                _iByteArray.Write (tempBytes, tempBytes.Length);
            }
