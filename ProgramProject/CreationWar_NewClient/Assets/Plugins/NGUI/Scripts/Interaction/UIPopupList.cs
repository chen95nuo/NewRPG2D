...
public class UIPopupList : UIWidgetContainer
{
	...
	[HideInInspector][SerializeField] public GameObject eventReceiver;
	[HideInInspector][SerializeField] public string functionName = "OnSelectionChange";
	...
	void OnClick()
	{
		if (enabled && NGUITools.GetActive(gameObject) && mChild == null && atlas != null && isValid && items.Count > 0)
		{
			...
			for (int i = 0, imax = items.Count; i < imax; ++i)
			{
				...
				lbl.text = isLocalized ? StaticLoc.Loc.Get(s) : s;
				lbl.color = textColor;
