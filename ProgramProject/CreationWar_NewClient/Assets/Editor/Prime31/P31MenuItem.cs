...
public class P31MenuItem : MonoBehaviour
{
	...
	[MenuItem( "Prime31/Plugin troubleshooter...")]
	static void troubleshooter()
	{
		// First up, grab all the PostprocessBuildPlayer* scripts
		string path = Path.Combine( Application.dataPath, "Editor" );
		DirectoryInfo dirInfo = new DirectoryInfo( path );
								 
		var postProcessScripts = from file in dirInfo.GetFiles( "PostprocessBuildPlayer*" )
								 where file.Extension != ".META"
								 select file;
								 
		var foundFileWithoutExectuable = false;
		
		// loop through and make sure they are all executable		
		foreach( FileInfo file in postProcessScripts )
		{
			using( var pr = new Process() )
			{
				pr.StartInfo.FileName = "ls";
				pr.StartInfo.Arguments = "-la " + file.FullName;
				...
				if( result.Substring( 3, 1 ) != "x" )
				{
					foundFileWithoutExectuable = true;
					break;
				}
			}
		}
		...
		// show an error
		if( foundFileWithoutExectuable )
			EditorUtility.DisplayDialog( "Found a Problem", "Please ensure all PostprocessBuildPlayer* files are executable!", "OK" );
		else
			EditorUtility.DisplayDialog( "All Clear", "No obvious problems found", "OK" );
	}
