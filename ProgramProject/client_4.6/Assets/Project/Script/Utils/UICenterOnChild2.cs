...
[AddComponentMenu("NGUI/Interaction/Center On Child2(vertical)")]
public class UICenterOnChild2 : MonoBehaviour
{
	/// <summary>
	...
	public void Recenter ()
	{
		...
		if (closest != null)
		{
			...
			/**每次滑动超过行高的一半就换行 lt@2013-11-27**/
			Vector3 temp=dt.localPosition - offset;
			float cellHeight=gameObject.GetComponent<UIGrid2>().cellHeight;
			float mul=dt.localPosition.y/cellHeight;
			float half=mul-(int)mul;
			float y=((int)mul+1)*cellHeight;
			if(half<0.5)
			{
				y=((int)mul)*cellHeight;
			}
			Vector3 offset2=new Vector3(temp.x,y,temp.z);
