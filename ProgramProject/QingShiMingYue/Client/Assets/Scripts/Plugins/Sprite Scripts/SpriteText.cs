...
public class SpriteText : MonoBehaviour, IUseCamera
{
	...
	/// <summary>
	/// The key to localize text, keep empty if the text need not localizing.
	/// </summary>
	public string localizingKey = "";
	...
	/// of a line.(// Should be the percentage of line height, not the the character height [12/11/2011 sonilics])
	/// </summary>
	...
	/// <summary>
	/// The name of the font to be used, it's defined in FontManager 
	/// </summary>
	[HideInInspector]
	...
	/// <summary>
	/// When set to a value over 0, and multiline set value
	/// true,over the maxLines values of Text while display 
	/// as overMaxLinesValue
	/// </summary>
	public int maxLines = 0;
	[HideInInspector]
	public string overMaxLinesValue = "......";
	...
	// The SpriteFontBase itself
	protected SpriteFontBase spriteFont;
	...
	protected SubMeshData[] subMeshDatas;
	...
	// Reset material flag
	protected bool resetMaterial = false;

	// Used to delay mesh updating for dynamic font
	protected bool delayUpdateMesh = false;
	...
	protected virtual void Init()
	{
		...
		// Get a default font if it isn't set
		if (fontName == "" && FontManager.instance != null)
			fontName = FontManager.instance.GetDefaultFontName();

		// Get the font:
		if (fontName != "")
		{
			spriteFont = FontManager.instance.GetFont(fontName);
			if (spriteFont == null)
				spriteFont = FontManager.instance.GetDefaultFont();
			if (spriteFont == null)
				Debug.LogWarning("Warning: " + name + " was unable to load font \"" + fontName + "\"!");
		}
		else
		{
			spriteFont = null;
		...
		// Update the game objects to render font
		// Force reset materials
		resetMaterial = true;
		UpdateFontMesh();
	...
	private bool IsEnglishLetter(int code)
	{
	...
	protected void ProcessString(string str)
	{
		...
		float maxLinesFilterWidth = 0f;

		for (int i = 0; i < overMaxLinesValue.Length; i++)
			maxLinesFilterWidth += (i == 0 ? spriteFont.GetAdvance(overMaxLinesValue[i]) : spriteFont.GetWidth(overMaxLinesValue[i - 1], overMaxLinesValue[i])) * worldUnitsPerTexel * characterSpacing;
			...
			if (maxWidth > 0 && multiline)
			{
				...
				for (int i = 0; i < str.Length; ++i)
				{
					if (char.IsWhiteSpace(str[i]))
					{
						...
						//and continue to new letter
						//if not continue will have wrong spaceleft in the next line
						if (str[i] == '\n')
						{
							spaceLeft = (maxWidthInPixels) ? (maxWidth * worldUnitsPerScreenPixel) : maxWidth;
					...
					if (spaceLeft < 0)
					{
						...
						// if isChineseLetter allways do this case
						if (spaceLeft < 0 || !IsEnglishLetter(str[i]))
						{
					...
					if (maxLines > 0 && newLineInserts.Count == maxLines)
					{
						newLineInserts.Sort((l1, l2) =>
							{
								return l1.index - l2.index;
							});

						int j = displaySB.ToString().LastIndexOf(newLineDelimiter[0]);
						spaceLeft = (maxWidthInPixels) ? (maxWidth * worldUnitsPerScreenPixel) : maxWidth;
						newLineInserts.RemoveAt(newLineInserts.Count - 1);

						displaySB.Remove(j, displaySB.Length - j);
						j = Mathf.Max(0, displaySB.ToString().LastIndexOf(newLineDelimiter[0]) + 1);

						for (; j < displaySB.Length; j++)
						{
							if (j <= 0 || displaySB[j - 1] == newLineDelimiter[0])
								spaceLeft -= spriteFont.GetAdvance(displaySB[j]) * worldUnitsPerTexel * characterSpacing;
							else
								spaceLeft -= spriteFont.GetWidth(displaySB[j - 1], displaySB[j]) * worldUnitsPerTexel * characterSpacing;

							if (spaceLeft <= maxLinesFilterWidth)
								break;
						}

						if (j < displaySB.Length)
							displaySB.Remove(j, displaySB.Length - j);

						displaySB.Append(overMaxLinesValue);
			...
			else // Else it's just another character:
			{
				...
				if (maxWidth > 0 && multiline)
				{
					if (char.IsWhiteSpace(str[strOffset]))
					{
						// If it's a newline, reset our space left:
						//and continue to new letter
						//if do not continue will have wrong spaceleft in the next line
						if (str[strOffset] == '\n')
						{
							spaceLeft = (maxWidthInPixels) ? (maxWidth * worldUnitsPerScreenPixel) : maxWidth;
							//lastWhiteSpace = displaySB.Length;
					...
					if (spaceLeft < 0)
					{
						...
						// too long, so clip it at our width:
						// if isChineseLetter allways do this case
						if (spaceLeft < 0 || !IsEnglishLetter(str[strOffset]))
						{
							// Insert a line break before the most recent character
							// which put us over the top:
							if (displaySB.Length > 0)
							{
								...
								newLineInserts.Add(new NewlineInsertInfo(displaySB.Length - 1, 1));
					...
					if (maxLines > 0 && newLineInserts.Count == maxLines)
					{
						...
						// Calculate index for remove over chars.
						for (; j < displaySB.Length; j++)
						{
							if (j <= 0 || displaySB[j - 1] == newLineDelimiter[0])
								spaceLeft -= spriteFont.GetAdvance(displaySB[j]) * worldUnitsPerTexel * characterSpacing;
						...
						if (j < displaySB.Length)
						{
							...
							if (j > 0)
							{
								for (int k = 0; k < colorInserts.Count; k++)
									if (colorInserts[k] >= j)
									{
										colorInserts.RemoveRange(k, colorInserts.Count - k);
										break;
									}
							}
						}
	...
	public void UpdateMesh()
	{
		...
		if (stringContentChanged)
		{
			bool waitForRenderData = false;

			for (int i = 0; i < displayString.Length; ++i)
				waitForRenderData |= spriteFont.GetSpriteChar(displayString[i]).waitForRenderData;

			if (waitForRenderData)
			{
				//Debug.Log("Wait delayUpdateMesh");
				delayUpdateMesh = true;
				return;
			}

			UpdateFontMesh();

			delayUpdateMesh = false;
		}
		...
		mesh.subMeshCount = subMeshDatas.Length;
		for (int i = 0; i < subMeshDatas.Length; ++i)
		{
			mesh.SetTriangles(subMeshDatas[i].faces.ToArray(), i);
		}
#if WANT_NORMALS
		if(vertCountChanged)
	...
	protected void Layout_Single_Line()
	{
		...
		// Clear subMesh setting
		foreach (var subMeshData in subMeshDatas)
			subMeshData.faces.Clear();
	...
	protected void BuildCharacter(int vertNum, int charNum, Vector3 upperLeft, ref SpriteChar ch)
	{
		...
		// Set index buffer
		subMeshDatas[ch.page].faces.Add(vertNum + 0);	//	0_ 1			0 ___ 3
		subMeshDatas[ch.page].faces.Add(vertNum + 3);	//  | /		Verts:	 |	/|
		subMeshDatas[ch.page].faces.Add(vertNum + 1);	// 2|/				1|/__|2

		subMeshDatas[ch.page].faces.Add(vertNum + 3);	//	  3
		subMeshDatas[ch.page].faces.Add(vertNum + 2);	//   /|
		subMeshDatas[ch.page].faces.Add(vertNum + 1);	// 5/_|4
	...
	{
		SetFont(FontManager.instance.GetFont(newFontName));
	}

	/// <summary>
	/// Changes the font to be used for this text object.
	/// </summary>
	/// <param name="newFont">Reference to the SpriteFontBase to be used for this text object.</param>
	/// <param name="fontMaterial">The material to use for the new font.</param>
	public void SetFont(SpriteFontBase newFont)
	{
	...
	public void UpdateFontMesh(bool forceResetMaterial)
	{
		if (forceResetMaterial)
			resetMaterial = true;
	...
	/// <summary>
	/// Update sub font-render object to fit materials changing, especially for dynamic fonts
	/// </summary>
	public void UpdateFontMesh()
	{
		int fontMaterialCount = spriteFont != null ? spriteFont.GetMaterialCount() : 0;

		// Update materials
		if (resetMaterial || meshRenderer.sharedMaterials.Length != fontMaterialCount)
		{
			Material[] materials = new Material[fontMaterialCount];
			for (int i = 0; i < fontMaterialCount; ++i)
			{
				materials[i] = spriteFont.GetMaterial(i);
			}

			// Set materials
			meshRenderer.sharedMaterials = materials;

			//Debug.Log("sharedMaterials count : " + meshRenderer.sharedMaterials.Length);

			resetMaterial = false;
		}

		// Update sub-mesh data
		if (subMeshDatas == null || subMeshDatas.Length != fontMaterialCount)
		{
			SubMeshData[] oldSubMeshDatas = subMeshDatas;
			subMeshDatas = new SubMeshData[fontMaterialCount];

			for (int i = 0; i < subMeshDatas.Length; ++i)
			{
				// Use the old class if it's impossible.
				if (oldSubMeshDatas != null && i < oldSubMeshDatas.Length)
					subMeshDatas[i] = oldSubMeshDatas[i];
				else
					subMeshDatas[i] = new SubMeshData();

				subMeshDatas[i].index = i;
				subMeshDatas[i].faces.Clear();
			}
		}
	...
	// Sets the number of pixels per UV unit:
	public void SetPixelToUV(SpriteFontBase spriteFont)
	{
		if (spriteFont == null)
			return;

		SetPixelToUV(spriteFont.TexWidth, spriteFont.TexHeight);
	}
	...
	{
		return lineSpacing * spriteFont.LineHeight * worldUnitsPerTexel;
	}
...
public class SpriteTextMirror
{
	...
	*/
	...
	public virtual void Mirror(SpriteText s)
	{
		...
		maxLines = s.maxLines;
		overMaxLinesValue = s.overMaxLinesValue;
		pixelPerfect = s.pixelPerfect;
	...
	public virtual bool DidChange(SpriteText s)
	{
		...
		if (maxLines != s.maxLines)
			return true;
		if (overMaxLinesValue != s.overMaxLinesValue)
			return true;
