...
public class SimpleSprite : SpriteRoot
{
	...
	/// <summary>
	/// Keep lowerLeftPixel, pixelDimensions use texture setting
	/// </summary>
	public bool useTextureSize = true;
	...
	public override void Copy(SpriteRoot s)
	{
		...
		useTextureSize = ((SimpleSprite)s).useTextureSize;
	...
	public override void InitUVs()
	{
		if (useTextureSize)
		{
			// Reset texture in editor mode
			if (Application.isPlaying == false && !managed)
				((SpriteMesh)m_spriteMesh).material = renderer != null && renderer.sharedMaterial != null ? renderer.sharedMaterial : null;

			if (m_spriteMesh.texture != null)
			{
				lowerLeftPixel.x = 0;
				lowerLeftPixel.y = m_spriteMesh.texture.height;

				pixelDimensions.x = m_spriteMesh.texture.width;
				pixelDimensions.y = m_spriteMesh.texture.height;
			}
		}
...
public class SimpleSpriteMirror : SpriteRootMirror
{
	...
	public override bool DidChange(SpriteRoot s)
	{
		...
		if (((SimpleSprite)s).useTextureSize != useTextureSize)
		{
			s.uvsInitialized = false;
			return true;
		}
