...
//	All rights reserved
//-----------------------------------------------------------------
//#define ENABLE_SCREEN_PLACEMENT
...
public abstract class SpriteRoot : MonoBehaviour, IEZLinkedListItem<ISpriteAnimatable>, IUseCamera
{
	...
	// Data members:
	//---------------------------------------
	// Template Support [5/22/2013 sonilics]
	[HideInInspector]
	...
	// Grid sprite support [12/17/2011 sonilics]
	/// <summary>
	...
	/// </summary>
	[HideInInspector]
	public bool xTextureTile = false;
	[HideInInspector]
	public bool yTextureTile = false;
	[HideInInspector]
	...
	[HideInInspector]
	public bool keepBorderAspectRate = false;
	[HideInInspector]
	public bool piercedStyle = false;
	...
	*/
	...
	// Grid sprite support [12/17/2011 sonilics]
	protected Vector2 scaledTopLeftBorder;
	protected Vector2 scaledBottomRightBorder;
	protected Vector2 unclippedTopLeftBorder;
	protected Vector2 unclippedBottomRightBorder;
	protected Vector2 topLeftBorder;
	protected Vector2 bottomRightBorder;
	protected bool clipped = false;				// Keeps track of whether we are to be clipped by a bounding box
	// tile support [5/10/2013 sonilics]
	protected Vector2 tileSize;
	// Buffers for tile calculating
	protected static float[] sUnclippedVertexValueBufferX, sUnclippedVertexValueBufferY;
	protected static float[] sVertexValueBufferX, sVertexValueBufferY;
	protected static float[] sUnclippedUVValueBufferX, sUnclippedUVValueBufferY;
	protected static float[] sUVValueBufferX, sUVValueBufferY;
	...
	protected bool m_Awake = false;				// This gets set when the sprite has entered Awake()
	...
	public virtual void Copy(SpriteRoot s)
	{
		...
		xTextureTile = s.xTextureTile;
		yTextureTile = s.yTextureTile;
		leftBorder = s.leftBorder;
		rightBorder = s.rightBorder;
		topBorder = s.topBorder;
		bottomBorder = s.bottomBorder;
		keepBorderAspectRate = s.keepBorderAspectRate;
		piercedStyle = s.piercedStyle;
	...
	public void CalcEdges()
	{
		switch (anchor)
		{
			case ANCHOR_METHOD.TEXTURE_OFFSET:
				...
				/*
								...
								{
									topLeftOffset.i = bottomRightOffset.i - (bottomRightOffset.i - topLeftOffset.i) * tlTruncate.i;
									topLeftOffset.i = bottomRightOffset.i - (bottomRightOffset.i - topLeftOffset.i) * tlTruncate.i;
									bottomRightOffset.i = topLeftOffset.i - (topLeftOffset.i - bottomRightOffset.i) * brTruncate.i;
									bottomRightOffset.i = topLeftOffset.i - (topLeftOffset.i - bottomRightOffset.i) * brTruncate.i;
								}
				*/
		...
		Vector2 unclipedImageSize = new Vector2(frameInfo.uvs.width * pixelsPerUV.x, frameInfo.uvs.height * pixelsPerUV.y);
		Vector2 imageScale = new Vector2(unclipedImageSize.x / frameInfo.textureSize.x, unclipedImageSize.y / frameInfo.textureSize.y);

		scaledTopLeftBorder = new Vector2(leftBorder * imageScale.x, topBorder * imageScale.y);
		scaledBottomRightBorder = new Vector2(rightBorder * imageScale.x, bottomBorder * imageScale.y);

		unclippedTopLeftBorder = new Vector2(leftBorder, topBorder) / UIManager.instance.textureResolutionScale;
		unclippedBottomRightBorder = new Vector2(rightBorder, bottomBorder) / UIManager.instance.textureResolutionScale;

		if (keepBorderAspectRate)
		{
			if (scaledTopLeftBorder.x == 0 && scaledBottomRightBorder.x == 0)
			{
				if (scaledTopLeftBorder.y != 0)
					unclippedTopLeftBorder.y = scaledTopLeftBorder.y * width / unclipedImageSize.x;
				if (scaledBottomRightBorder.y != 0)
					unclippedBottomRightBorder.y = scaledBottomRightBorder.y * width / unclipedImageSize.x;
			}

			if (scaledTopLeftBorder.y == 0 && scaledBottomRightBorder.y == 0)
			{
				if (scaledTopLeftBorder.x != 0)
					unclippedTopLeftBorder.x = scaledTopLeftBorder.x * height / unclipedImageSize.y;
				if (scaledBottomRightBorder.x != 0)
					unclippedBottomRightBorder.x = scaledBottomRightBorder.x * height / unclipedImageSize.y;
			}
		}


		// Calculate clipped border size;
		topLeftBorder = unclippedTopLeftBorder;
		bottomRightBorder = unclippedBottomRightBorder;

		if (truncated || clipped)
		{
			topLeftBorder.x = Mathf.Clamp(unclippedTopLeftBorder.x - (topLeft.x - unclippedTopLeft.x), 0, Mathf.Min(unclippedTopLeftBorder.x, bottomRight.x - topLeft.x));
			topLeftBorder.y = Mathf.Clamp(unclippedTopLeftBorder.y - (unclippedTopLeft.y - topLeft.y), 0, Mathf.Min(unclippedTopLeftBorder.y, topLeft.y - bottomRight.y));
			bottomRightBorder.x = Mathf.Clamp(unclippedBottomRightBorder.x - (unclippedBottomRight.x - bottomRight.x), 0, Mathf.Min(unclippedBottomRightBorder.x, bottomRight.x - topLeft.x));
			bottomRightBorder.y = Mathf.Clamp(unclippedBottomRightBorder.y - (bottomRight.y - unclippedBottomRight.y), 0, Mathf.Min(unclippedBottomRightBorder.y, topLeft.y - bottomRight.y));
		}

		// Calculate tile
		if (xTextureTile || yTextureTile)
		{
			Vector2 originalCenterImageSize = new Vector2(frameInfo.textureSize.x - leftBorder - rightBorder, frameInfo.textureSize.y - topBorder - bottomBorder);

			if (xTextureTile && yTextureTile)
			{
				tileSize.x = originalCenterImageSize.x / UIManager.instance.textureResolutionScale;
				tileSize.y = originalCenterImageSize.y / UIManager.instance.textureResolutionScale;
			}
			else if (xTextureTile)
			{
				tileSize.y = height;
				tileSize.x = keepBorderAspectRate ? originalCenterImageSize.x * tileSize.y / frameInfo.textureSize.y : originalCenterImageSize.x / UIManager.instance.textureResolutionScale;
			}
			else
			{
				tileSize.x = width;
				tileSize.y = keepBorderAspectRate ? originalCenterImageSize.y * tileSize.x / frameInfo.textureSize.x : originalCenterImageSize.y / UIManager.instance.textureResolutionScale;
			}
		}
	}

	public void CalcGridCount(out int gridCountX, out int gridCountY, out bool pierced)
	{
		gridCountX = 1;
		if (scaledTopLeftBorder.x != 0)
			gridCountX++;
		...
		if (xMirror)
			gridCountX++;
		else if (xTextureTile && tileSize.x != 0)
			gridCountX += (int)Mathf.Ceil((unclippedBottomRight.x - unclippedTopLeft.x - unclippedTopLeftBorder.x - unclippedBottomRightBorder.x) / tileSize.x) - 1;
		gridCountX = Mathf.Max(1, gridCountX);
		...
		if (yMirror)
			gridCountY++;
		else if (yTextureTile && tileSize.y != 0)
			gridCountY += (int)Mathf.Ceil((unclippedTopLeft.y - unclippedBottomRight.y - unclippedTopLeftBorder.y - unclippedBottomRightBorder.y) / tileSize.y) - 1;
		gridCountY = Mathf.Max(1, gridCountY);

		pierced = gridCountX > 2 && gridCountY > 2 && piercedStyle;
	}

	private static float[] GetBuffer(ref float[] buffer, int size)
	{
	...
	private void CalcUnclippedVertexGrids(out float[] xValues, out int xLength, out float[] yValues, out int yLength)
	{
		Vector3 _topLeft = unclippedTopLeft + offset;
		Vector3 _bottomRight = unclippedBottomRight + offset;
		Vector3 _topLeftBorder = unclippedTopLeftBorder;
		Vector3 _bottomRightBorder = unclippedBottomRightBorder;

		xValues = GetBuffer(ref sUnclippedVertexValueBufferX, xLength = m_spriteMesh.vertexCountX);
		yValues = GetBuffer(ref sUnclippedVertexValueBufferY, yLength = m_spriteMesh.vertexCountY);

		try
		{
			if (xTextureTile == false)
			{
				/*
				 * Calculate X values
				 */
				...
				// Start
				xValues[xIndex++] = _topLeft.x;

				// Left border
				if (scaledTopLeftBorder.x != 0)
					xValues[xIndex++] = xValues[xIndex - 2] + _topLeftBorder.x;

				// Middle Mirror.
				if(xMirror)
					xValues[xIndex++] = (_topLeft.x + _bottomRight.x) / 2;

				// Right border
				if (scaledBottomRightBorder.x != 0)
					xValues[xIndex++] = _bottomRight.x - _bottomRightBorder.x;

				// End
				xValues[xIndex++] = _bottomRight.x;
			}
			else
			{
				/*
				...
				// X tile			
				if (xTextureTile && tileSize.x != 0)
				{
					float xTileStart = unclippedTopLeft.x + unclippedTopLeftBorder.x;
					float xTileEnd = unclippedBottomRight.x - unclippedBottomRightBorder.x;
					while (xTileStart < xTileEnd)
					{
						if (paired)
						{
							xValues[xIndex++] = xValues[xIndex - 2];
							paired = !paired;
						}

						xValues[xIndex++] = Mathf.Clamp(xValues[xIndex - 2] + tileSize.x, _topLeft.x, _bottomRight.x - _bottomRightBorder.x);
						paired = !paired;

						xTileStart += tileSize.x;
					}

					xValues[xIndex - 1] = Mathf.Clamp(xTileEnd + tileSize.x, _topLeft.x, _bottomRight.x - _bottomRightBorder.x);
				}
				// Previous - Right border start
				else if (scaledBottomRightBorder.x != 0)
				{
					if (paired)
					{
						xValues[xIndex++] = xValues[xIndex - 2];
			...
			if (yTextureTile == false)
			{
				...
				// Start
				yValues[yIndex++] = _topLeft.y;

				// Top border
				if (scaledTopLeftBorder.y != 0)
					yValues[yIndex++] = yValues[yIndex - 2] - _topLeftBorder.y;

				// Middle mirror
				if (yMirror)
					yValues[yIndex++] = (_topLeft.y + _bottomRight.y) / 2;

				// Bottom border
				if (scaledBottomRightBorder.y != 0)
					yValues[yIndex++] = _bottomRight.y + _bottomRightBorder.y;

				// End
				yValues[yIndex++] = _bottomRight.y;
			}
			else
			{
				/*
				...
				// Tile
				if (yTextureTile && tileSize.y != 0)
				{
					float yTileStart = unclippedTopLeft.y - unclippedTopLeftBorder.y;
					float yTileEnd = unclippedBottomRight.y + unclippedBottomRightBorder.y;
					while (yTileStart > yTileEnd)
					{
						if (paired)
						{
							yValues[yIndex++] = yValues[yIndex - 2];
							paired = !paired;
						}

						yValues[yIndex++] = Mathf.Clamp(yValues[yIndex - 2] - tileSize.y, _bottomRight.y + _bottomRightBorder.y, _topLeft.y);
						paired = !paired;

						yTileStart -= tileSize.y;
					}

					yValues[yIndex - 1] = Mathf.Clamp(yValues[yIndex - 1], _bottomRight.y + _bottomRightBorder.y, _topLeft.y);
				}
				// Previous - bottom border
				else if (scaledBottomRightBorder.y != 0)
				{
					if (paired)
					{
						yValues[yIndex++] = yValues[yIndex - 2];
		...
		{
			Debug.LogError(this.gameObject.name + " " + e.Message);
		}
	}

	private void CalcVertexGrids(out float[] xValues, out int xLength, out float[] yValues, out int yLength)
	{
		float[] xUnclippedValues, yUnclippedValues;
		CalcUnclippedVertexGrids(out xUnclippedValues, out xLength, out yUnclippedValues, out yLength);

		CalcVertexGrids(xUnclippedValues, xLength, yUnclippedValues, yLength, out xValues, out yValues);
	}

	private void CalcVertexGrids(float[] xUnclippedValues, int xLength, float[] yUnclippedValues, int yLength, out float[] xValues, out float[] yValues)
	{
		xValues = GetBuffer(ref sVertexValueBufferX, xLength);
		yValues = GetBuffer(ref sVertexValueBufferY, yLength);

		for (int x = 0; x < xLength; ++x)
			xValues[x] = Mathf.Clamp(xUnclippedValues[x], topLeft.x, bottomRight.x);

		for (int y = 0; y < yLength; ++y)
			yValues[y] = Mathf.Clamp(yUnclippedValues[y], bottomRight.y, topLeft.y);
	}

	private void CalcUnclippedUVGrids(out float[] xValues, out int xLength, out float[] yValues, out int yLength)
	{
		...
		Vector2 topLeftBorderUV = PixelSpaceToUVSpace(scaledTopLeftBorder);
		Vector2 bottomRightBorderUV = PixelSpaceToUVSpace(scaledBottomRightBorder);

		xValues = GetBuffer(ref sUnclippedUVValueBufferX, xLength = m_spriteMesh.vertexCountX);
		yValues = GetBuffer(ref sUnclippedUVValueBufferY, yLength = m_spriteMesh.vertexCountY);
		...
		try
		{
			if (xTextureTile == false)
			{
				...
				// Start
				xValues[xIndex++] = baseUV.x;

				// Left border
				if (scaledTopLeftBorder.x != 0)
					xValues[xIndex++] = baseUV.x + topLeftBorderUV.x;

				if (xMirror)
				{
					// Middle mirror
					xValues[xIndex++] = baseUV.xMax;
				...
				else
				{
					// Right border
					if (scaledBottomRightBorder.x != 0)
						xValues[xIndex++] = baseUV.xMax - bottomRightBorderUV.x;
			...
			else
			{
				...
				if (xTextureTile && tileSize.x != 0)
				{
					...
					if (xTileStart != xTileEnd)
						xValues[xIndex - 1] = Mathf.Lerp(baseUV.x + topLeftBorderUV.x, baseUV.xMax - bottomRightBorderUV.x, Mathf.Repeat(xTileEnd - (xTileStart - tileSize.x), tileSize.x) / tileSize.x);
				}
				else if (scaledBottomRightBorder.x != 0)
				{
					if (paired == false)
			...
			if (yTextureTile == false)
			{
				...
				// Start
				yValues[yIndex++] = baseUV.yMax;

				// Top border
				if (scaledTopLeftBorder.y != 0)
					yValues[yIndex++] = baseUV.yMax - topLeftBorderUV.y;

				if (yMirror)
				{
					// Middle mirror
					if (yMirror)
						yValues[yIndex++] = baseUV.y;
				...
				else
				{
					// Right border
					if (scaledBottomRightBorder.y != 0)
						yValues[yIndex++] = baseUV.y + bottomRightBorderUV.y;
			...
			else
			{
				...
				if (yTextureTile && tileSize.y != 0)
				{
					...
					if (yTileStart != yTileEnd)
						yValues[yIndex - 1] = Mathf.Lerp(baseUV.yMax - topLeftBorderUV.y, baseUV.y + bottomRightBorderUV.y, Mathf.Repeat(yTileStart + tileSize.y - yTileEnd, tileSize.y) / tileSize.y);
				}
				else if (scaledBottomRightBorder.y != 0)
				{
					if (paired == false)
	...
	private void CalcUVGrids(out float[] xValues, out int xLength, out float[] yValues, out int yLength)
	{
		// Get unclipped vertex values
		float[] xUnclippedVertexValues, yUnclippedVertexValues;
		CalcUnclippedVertexGrids(out xUnclippedVertexValues, out xLength, out yUnclippedVertexValues, out yLength);

		// Get clipped vertex values
		float[] xVertexValues, yVertexValues;
		CalcVertexGrids(xUnclippedVertexValues, xLength, yUnclippedVertexValues, yLength, out xVertexValues, out yVertexValues);

		// Get unclipped UV values
		float[] xUnclippedUVValues, yUnclippedUVValues;
		CalcUnclippedUVGrids(out xUnclippedUVValues, out xLength, out yUnclippedUVValues, out yLength);

		// Calculate UV
		xValues = GetBuffer(ref sUVValueBufferX, xLength);
		yValues = GetBuffer(ref sUVValueBufferY, yLength);

		for (int x = 0; x < xLength; ++x)
		{
			if (x == 0 || (x != xLength - 1 && xVertexValues[x] >= xUnclippedVertexValues[x]))
			{
				float span = xUnclippedVertexValues[x + 1] - xUnclippedVertexValues[x];
				if (span == 0)
					xValues[x] = xUnclippedUVValues[x];
				else
					xValues[x] = Mathf.Lerp(xUnclippedUVValues[x], xUnclippedUVValues[x + 1], (xVertexValues[x] - xUnclippedVertexValues[x]) / span);
			}
			else
			{
				float span = xUnclippedVertexValues[x] - xUnclippedVertexValues[x - 1];
				if (span == 0)
					xValues[x] = xUnclippedUVValues[x];
				else
					xValues[x] = Mathf.Lerp(xUnclippedUVValues[x - 1], xUnclippedUVValues[x], (xVertexValues[x] - xUnclippedVertexValues[x - 1]) / span);
			}
		}

		for (int y = 0; y < yLength; ++y)
		{
			if (y == 0 || (y != yLength - 1 && yVertexValues[y] <= yUnclippedVertexValues[y]))
			{
				float span = yUnclippedVertexValues[y + 1] - yUnclippedVertexValues[y];
				if (span == 0)
					yValues[y] = yUnclippedUVValues[y];
				else
					yValues[y] = Mathf.Lerp(yUnclippedUVValues[y], yUnclippedUVValues[y + 1], (yVertexValues[y] - yUnclippedVertexValues[y]) / (yUnclippedVertexValues[y + 1] - yUnclippedVertexValues[y]));
			}
			else
			{
				float span = yUnclippedVertexValues[y] - yUnclippedVertexValues[y - 1];
				if (span == 0)
					yValues[y] = yUnclippedUVValues[y];
				else
					yValues[y] = Mathf.Lerp(yUnclippedUVValues[y - 1], yUnclippedUVValues[y], (yVertexValues[y] - yUnclippedVertexValues[y - 1]) / (yUnclippedVertexValues[y] - yUnclippedVertexValues[y - 1]));
			}
		}
	}
	...
	{
		CalcEdges();
		bool faceSizeChanged = m_spriteMesh.UpdateBuffers();

		Vector3[] vertices = m_spriteMesh.vertices;

		int xLength, yLength;
		float[] xVertexValues, yVertexValues;
		CalcVertexGrids(out xVertexValues, out xLength, out yVertexValues, out yLength);
		...
		for (int y = 0, index = 0; y < yLength; ++y)
		{
			...
			{
				vertices[index].x = xVertexValues[x];
				vertices[index].y = yVertexValues[y];
				vertices[index].z = offset.z;
			}
		}
	...
	public virtual void UpdateUVs()
	{
		...
		int xLength, yLength;
		float[] xUVValues, yUVValues;
		if (clipped || truncated)
			CalcUVGrids(out xUVValues, out xLength, out yUVValues, out yLength);
		else
			CalcUnclippedUVGrids(out xUVValues, out xLength, out yUVValues, out yLength);

		for (int y = 0, index = 0; y < yLength; ++y)
		{
			for (int x = 0; x < xLength; ++x, ++index)
			{
				uvs[index].x = xUVValues[x];
				uvs[index].y = yUVValues[y];
			}
		}
	...
	public Vector3 GetCenterPoint()
	{
		...
		{
			case SPRITE_PLANE.XY:
				return new Vector3(verts[0].x + 0.5f * (verts[last].x - verts[0].x), verts[0].y - 0.5f * (verts[0].y - verts[last].y), offset.z);
			case SPRITE_PLANE.XZ:
				return new Vector3(verts[0].x + 0.5f * (verts[last].x - verts[0].x), offset.y, verts[0].z - 0.5f * (verts[0].z - verts[last].z));
			case SPRITE_PLANE.YZ:
				return new Vector3(offset.x, verts[0].y - 0.5f * (verts[0].y - verts[last].y), verts[0].z - 0.5f * (verts[0].z - verts[last].z));
			default:
				return new Vector3(verts[0].x + 0.5f * (verts[last].x - verts[0].x), verts[0].y - 0.5f * (verts[0].y - verts[last].y), offset.z);
		}
	}
	...
	public Vector3 BottomRight
	{
		...
		{
			if (m_spriteMesh != null)
				return m_spriteMesh.vertices[m_spriteMesh.vertices.Length - 1];
			else
				return Vector3.zero;
		}
	}
...
public class SpriteRootMirror
{
	...
	public bool xTextureTile, yTextureTile;
	public bool xMirror, yMirror;
	public int leftGrid, rightGrid, topGrid, bottomGrid;
	public bool keepTextureScale, piercedStyle;
	...
	*/
	...
	public virtual void Mirror(SpriteRoot s)
	{
		...
		xMirror = s.xMirror;
		yMirror = s.yMirror;
		leftGrid = s.leftBorder;
		rightGrid = s.rightBorder;
		topGrid = s.topBorder;
		bottomGrid = s.bottomBorder;
		keepTextureScale = s.keepBorderAspectRate;
		piercedStyle = s.piercedStyle;
		bleedCompensation = s.bleedCompensation;
		anchor = s.anchor;
		offset = s.offset;
	...
	public virtual bool DidChange(SpriteRoot s)
	{
		...
		if (s.height != height)
			return true;
		if (s.xTextureTile != xTextureTile)
			return true;
		if (s.yTextureTile != yTextureTile)
			return true;
		if (s.xMirror != xMirror)
			return true;
		if (s.yMirror != yMirror)
			return true;
		if (s.leftBorder != leftGrid)
			return true;
		if (s.rightBorder != rightGrid)
			return true;
		if (s.topBorder != topGrid)
			return true;
		if (s.bottomBorder != bottomGrid)
			return true;
		if (s.keepBorderAspectRate != keepTextureScale)
			return true;
		if (s.piercedStyle != piercedStyle)
			return true;
