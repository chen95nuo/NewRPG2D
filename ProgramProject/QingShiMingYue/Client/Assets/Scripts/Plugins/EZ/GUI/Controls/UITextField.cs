...
public class UITextField : AutoSpriteControlBase, IKeyFocusable
{
	...
	// place holder support [sonilics]
	public Color placeHolderColorTag = Color.white;
...
*/
	...
	public override bool GotFocus()
	{
		...
		// Remove textPlaceHolder [sonilics]
		if (hasFocus == true && placeHolder != "" && Content == "" && spriteText != null)
		{
	...
	public string GetInputText(ref KEYBOARD_INFO info)
	{
		...
		info.textPlaceHolder = placeHolder;
#endif
		// Show our caret
		ShowCaret();
...
*/
	...
	public void LostFocus()
	{
		...
		// Set textPlaceHolder [sonilics]
		if (placeHolder != "" && Content == "" && spriteText != null)
		{
			spriteText.parseColorTags = true;
			spriteText.password = false;
			spriteText.Text = placeHolderColorTag + placeHolder;
		}

		// Make text shown at front when lost focus [sonilics]
		if (spriteText != null)
		{
			// Reset sprite text postion
			spriteText.transform.localPosition = origTextPos;
	...
	public void Commit()
	{
		if (scriptWithMethodToInvoke != null && !string.IsNullOrEmpty(methodToInvoke))
		{
			...
			if (UIObjectEx.InvokeMethod(scriptWithMethodToInvoke, methodToInvoke, this) == false)
	...
	public void SetInsertPosition(int _insert)
	{
		...
		if (multiline)
		{
			...
			switch (spriteText.anchor)
			{
				...
				case SpriteText.Anchor_Pos.Lower_Center:
					// [Modified by kingper] Fixed clip width bug.
					distanceToMargin = (marginBottomRight.x - marginTopLeft.x) - Mathf.Abs(origTextPos.x);
					break;
	...
	public override string Text
	{
		...
		set
		{
			...
			// Set remove textPlaceHolder [sonilics]
			if (hasFocus == false && Text == "" && placeHolder !="")
			{
				spriteText.parseColorTags = true;
				spriteText.password = false;
				spriteText.Text = placeHolderColorTag + placeHolder;
