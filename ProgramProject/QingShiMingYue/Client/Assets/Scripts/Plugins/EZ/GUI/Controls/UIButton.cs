...
public class UIButton : AutoSpriteControlBase
{
	...
	public SpriteText[] textLayers = new SpriteText[0];
		...
		if (Application.isPlaying)
		{
			...
			for (int i = 0; i < transitions.Length; ++i)
				for (int j = 0; j < transitions[i].list.Length; ++j)
				{
					...
					for (int k = 0; k < textLayers.Length; ++k)
					{
						if (textLayers[k] == null)
							continue;

						transitions[i].list[j].AddSubSubject(textLayers[k].gameObject);
					}
				}
