...
public class UIScrollList : MonoBehaviour, IUIObject
{
	...
	/// </summary>
	public bool positionItemsImmediately = false;
	...
	protected Queue<IUIListObject> clearQueue = new Queue<IUIListObject>(); // The list of items pending destruction [From EZ1.08]
	protected bool keepItemPosBeforeNewItem = true;
	...
	protected void Start()
	{
		...
		cachedRot = CachedTransform.rotation;
	...
	protected void CalcScreenToWorldUnits()
	{
		...
		float dist = nearPlane.GetDistanceToPoint(CachedTransform.position);
	...
	protected void CalcClippingRect()
	{
		...
		clientClippingRect.MultFast(CachedTransform.localToWorldMatrix);
	...
	public void ScrollToItem(IUIListObject item, float scrollTime, EZAnimation.EASING_TYPE easing, float offset)
	{
		snappedItem = item;
		...
		{
			// Fix bug : should calculate the start offset of items [2/07/2012 sonilics]
			if (direction == DIRECTION.TtoB_LtoR)
				//autoScrollPos = (item.transform.localPosition.x + offset - items[0].transform.localPosition.x) / amtOfPlay;
				autoScrollPos = Mathf.Clamp01((item.transform.localPosition.x + offset - items[0].transform.localPosition.x) / amtOfPlay);
			else
				//autoScrollPos = (-(item.transform.localPosition.x - offset) + items[0].transform.localPosition.x) / amtOfPlay;
				autoScrollPos = Mathf.Clamp01((-(item.transform.localPosition.x - offset) + items[0].transform.localPosition.x) / amtOfPlay);
		}
		else
		{
			// Fix bug : should calculate the start offset of items [2/07/2012 sonilics]
			if (direction == DIRECTION.TtoB_LtoR)
				//autoScrollPos = (-(item.transform.localPosition.y - offset) + items[0].transform.localPosition.y) / amtOfPlay;
				autoScrollPos = Mathf.Clamp01((-(item.transform.localPosition.y - offset) + items[0].transform.localPosition.y) / amtOfPlay);
			else
				//autoScrollPos = (item.transform.localPosition.y + offset - items[0].transform.localPosition.y) / amtOfPlay;
				autoScrollPos = Mathf.Clamp01((item.transform.localPosition.y + offset - items[0].transform.localPosition.y) / amtOfPlay);
		}
	...
	public void ScrollToItem(int index, float scrollTime, EZAnimation.EASING_TYPE easing)
	{
		if (index < 0 || index >= items.Count)
			return;
		ScrollToItem(items[index], scrollTime, easing, 0);
	}
	...
	{
		ScrollToItem(item, scrollTime, snapEasing, 0);
	}

	// Scroll with offest [sonilics]
	public void ScrollToItem(IUIListObject item, float scrollTime, float offset)
	{
		ScrollToItem(item, scrollTime, snapEasing, offset);
	}
	...
	{
		InsertItem(item, position, true, null, false);
	}
	...
	{
		InsertItem(item, position, true, null, doEasing);
	}
	...
	{
		InsertItem(item, position, true, text, false);
	}
	...
	/// <param name="text">Text to display in the item (requires that the item has a TextMesh associated with it, preferably in a child GameObject).</param>
	/// <param name="doEasing">Indicates whether easing should be performed to reposition other items in the list when making room for the inserted item.  Only applies if the item is being inserted before another item.</param>
	public void InsertItem(IUIListObject item, int position, bool keepPrevItemPosition, string text, bool doEasing)
	{
		// Unclip item as default of clipped item correct [3/05/2012 sonilics]
		item.Unclip();
		...
		{
			item.Hide(true);
			// Fix bug : Make Awake() of created component in this function can be called[7/15/2013 sonilics]
			//            if (!item.Managed)
			//#if UNITY_4_0 || UNITY_4_1 || UNITY_4_2 || UNITY_4_3 || UNITY_4_4 || UNITY_4_5 || UNITY_4_6 || UNITY_4_7 || UNITY_4_8 || UNITY_4_9
		...
		if (position != items.Count)
		{
			...
			{
				// Fix bug : When insert item in position 0 , it not add new item in the visibleItems.
				int prevIdx = item.Index > 0 ? visibleItems.IndexOf(items[item.Index - 1]) : -1;
				if (prevIdx == -1)
				{
		...
		// Save keepPrevItemPosition
		this.keepItemPosBeforeNewItem = keepPrevItemPosition;

		// See if we need to go ahead and position the item:
		if (positionItemsImmediately)
		{
			if (itemsInserted || doItemEasing)
				RepositionItems(keepPrevItemPosition);
			else
				PositionNewItems(keepPrevItemPosition);

			itemsInserted = false;
			keepItemPosBeforeNewItem = false;
			newItems.Clear();
		}
				...
				if(position == items.Count)
				{
					...
					if (orientation == ORIENTATION.HORIZONTAL)
					{
						...
						else
						{
							...
							if (direction == DIRECTION.TtoB_LtoR)
								x = viewableRectActual.x - item.TopLeftEdge.x + ((spacingAtEnds) ? (itemSpacing) : (0)) + extraEndSpacing;
							else
								x = viewableRectActual.xMax - item.BottomRightEdge.x - ((spacingAtEnds) ? (itemSpacing) : (0)) - extraEndSpacing;
						}
						...
						switch(alignment)
						{
							...
							case ALIGNMENT.LEFT_TOP:
								y = viewableRectActual.yMax - item.TopLeftEdge.y;
								break;
							case ALIGNMENT.RIGHT_BOTTOM:
								y = viewableRectActual.y - item.BottomRightEdge.y;
								break;
						}
					...
					else
					{
						...
						else
						{
							...
							if(direction == DIRECTION.TtoB_LtoR)
								y = viewableRectActual.yMax - item.TopLeftEdge.y - ((spacingAtEnds) ? (itemSpacing) : (0)) - extraEndSpacing;
							else
								y = viewableRectActual.y - item.BottomRightEdge.y + ((spacingAtEnds) ? (itemSpacing) : (0)) + extraEndSpacing;
						}
						...
						switch (alignment)
						{
							case ALIGNMENT.CENTER:
								...
								break;
							case ALIGNMENT.LEFT_TOP:
								x = viewableRectActual.x - item.TopLeftEdge.x;
								break;
							case ALIGNMENT.RIGHT_BOTTOM:
								x = viewableRectActual.xMax - item.BottomRightEdge.x;
								break;
						}
	...
	protected void PositionNewItems(bool keepItemPosBeforeScrollPos)
	{
		...
		for (int i = 0; i < newItems.Count; ++i)
		{
			...
			if (orientation == ORIENTATION.HORIZONTAL)
			{
				...
				else
				{
					...
					if (direction == DIRECTION.TtoB_LtoR)
						x = viewableRectActual.x - item.TopLeftEdge.x + ((spacingAtEnds) ? (itemSpacingActual) : (0)) + extraEndSpacingActual;
					else
						x = viewableRectActual.xMax - item.BottomRightEdge.x - ((spacingAtEnds) ? (itemSpacingActual) : (0)) - extraEndSpacingActual;
				}
			...
			else
			{
				...
				else
				{
					...
					if (direction == DIRECTION.TtoB_LtoR)
						y = viewableRectActual.yMax - item.TopLeftEdge.y - ((spacingAtEnds) ? (itemSpacingActual) : (0)) - extraEndSpacingActual;
					else
						y = viewableRectActual.y - item.BottomRightEdge.y + ((spacingAtEnds) ? (itemSpacingActual) : (0)) + extraEndSpacingActual;
				}
		...
		UpdateContentExtents(contentDelta, keepItemPosBeforeScrollPos ? 0 : contentDelta);
		ClipItems();
	...
	public void AddItem(IUIListObject item, string text)
	{
		...
		InsertItem(item, items.Count, true, text, false);
	}
	...
	public IUIListObject CreateItem(GameObject prefab, int position, string text, bool doEasing)
	{
		...
		*/
		...
		InsertItem(newItem, position, true, text, doEasing);
		return newItem;
	}
	...
	protected void UpdateContentExtents(float change, float scrollOffset)
	{
		...
		if (orientation == ORIENTATION.HORIZONTAL)
		{
			...
			amtOfPlay = ((contentExtents + addlSpacing) - viewableRectActual.width);
			scrollMax = (viewableRectActual.width / ((contentExtents + addlSpacing) - viewableRectActual.width)) * overscrollAllowance;
		}
		else
		{
			// Adjust the scroll position:
			amtOfPlay = ((contentExtents + addlSpacing) - viewableRectActual.height);
			scrollMax = (viewableRectActual.height / ((contentExtents + addlSpacing) - viewableRectActual.height)) * overscrollAllowance;
		}

		float newPos = ((oldAmtOfPlay * scrollPos + scrollOffset) / amtOfPlay);

		// [Modified by kinger]
		// Hide slider if contents is showed fully.
		if (slider != null && slider.spriteMesh != null && slider.spriteMesh is SpriteMesh)
		{
			SpriteMesh ms = (SpriteMesh)slider.spriteMesh;

			if (ms.mesh != null)
			{
				if (System.Double.IsNaN(scrollMax))
					slider.Hide(true);
				else
					slider.Hide(scrollMax <= 0);
			}
		}
	...
	{ return viewableRectActual.center.y; }

	protected float GetYAlignTop(IUIListObject item)
	{ return viewableRectActual.yMax - item.TopLeftEdge.y; }

	protected float GetYAlignBottom(IUIListObject item)
	{ return viewableRectActual.y - item.BottomRightEdge.y; }

	protected float GetXCentered(IUIListObject item)
	{ return viewableRectActual.center.x; }

	protected float GetXAlignLeft(IUIListObject item)
	{ return viewableRectActual.x - item.TopLeftEdge.x; }

	protected float GetXAlignRight(IUIListObject item)
	{ return viewableRectActual.xMax - item.BottomRightEdge.x; }
	//=====================================================
	...
	public void PositionItems(bool keepItemPosBeforeScrollPos)
	{
		...
		float oldContentExtents = contentExtents;
		if (orientation == ORIENTATION.HORIZONTAL)
			PositionHorizontally(false);
		else
			PositionVertically(false);

		UpdateContentExtents(0, keepItemPosBeforeScrollPos ? 0 : contentExtents - oldContentExtents);	// Just so other stuff gets updated
		ClipItems();
	...
	public void RepositionItems(bool keepItemPosBeforeScrollPos)
	{
		if (itemEasers.Count > 0)
		{
			// Cancel any pending easings:
	...
	protected void PositionHorizontally(bool updateExtents)
	{
		...
		if (direction == DIRECTION.TtoB_LtoR)
		{
			edge = viewableRectActual.x + ((spacingAtEnds) ? (itemSpacingActual) : (0)) + extraEndSpacingActual;
			...
			// Fix bug : Do not reduce item spacing when item count is zero [2/07/2012 sonilics]
			if (!spacingAtEnds && items.Count != 0)
				contentExtents -= itemSpacingActual;
		}
		else
		{
			edge = viewableRectActual.xMax - ((spacingAtEnds) ? (itemSpacingActual) : (0)) - extraEndSpacingActual;
	...
	protected void PositionVertically(bool updateExtents)
	{
		...
		if (direction == DIRECTION.TtoB_LtoR)
		{
			edge = viewableRectActual.yMax - ((spacingAtEnds) ? (itemSpacingActual) : (0)) - extraEndSpacingActual;
		...
		{
			edge = viewableRectActual.y + ((spacingAtEnds) ? (itemSpacingActual) : (0)) + extraEndSpacingActual;
	...
	protected void ClipItems()
	{
		...
		if (orientation == ORIENTATION.HORIZONTAL)
		{
			...
			float leftVisibleEdge = viewableRectActual.x - moverOffset;
			float rightVisibleEdge = viewableRectActual.xMax - moverOffset;
		...
		else
		{
			...
			float itemOffset;
			// Calculate the visible edges inside the mover's local space:
			float topVisibleEdge = viewableRectActual.yMax - moverOffset;
			float bottomVisibleEdge = viewableRectActual.y - moverOffset;
	...
	public void DidClick(IUIObject item)
	{
		...
		if (scriptWithMethodToInvoke != null)
		{
			// [Modify by kingper] Invoke method to pass current object to callback function.
			// All controls will use use method for invoking callback.
			if (UIObjectEx.InvokeMethod(scriptWithMethodToInvoke, methodToInvokeOnSelect, this) == false)
				scriptWithMethodToInvoke.Invoke(methodToInvokeOnSelect, 0);
		}
		if (changeDelegate != null)
			changeDelegate(this);
	...
	public void ListDragged(POINTER_INFO ptr)
	{
		...
		inputPoint1 = CachedTransform.InverseTransformPoint(inputPoint1);
		inputPoint2 = CachedTransform.InverseTransformPoint(inputPoint2);
	...
	{
		RemoveItem(index, destroy, true, false);
	}
	...
	public void RemoveItem(int index, bool destroy, bool keepPrevItemPosition, bool doEasing)
	{
		if (index < 0 || index >= items.Count)
			return;
		...
		{
			// Game object pool support [sonilics]
			if (items[index].GameObjectPool != null)
			{
				// Move to pool
				items[index].GameObjectPool.ReleaseItem(items[index].gameObject);
			}
			else
			{
		...
		PositionItems(keepPrevItemPosition);
	}
	...
	/// <param name="destroy">Whether or not to destroy the item being removed.</param>
	/// <param name="doEasing">Indicates whether easing should be performed on the items which will move to fill the void created by removal of this item. Only applies if the item being removed is not at the end of the list.</param>
	public void RemoveItem(IUIListObject item, bool destroy, bool keepPrevItemPosition, bool doEasing)
	{
		for (int i = 0; i < items.Count; ++i)
		{
			if (items[i] == item)
			{
				RemoveItem(i, destroy, keepPrevItemPosition, doEasing);
				return;
			}
		}
	}
	...
	public void ClearList(bool destroy)
	{
		...
		for (int i = 0; i < items.Count; ++i)
		{
			...
			// Game object pool support [sonilics]
			else if (items[i].GameObjectPool != null)
	...
	public void LateUpdate()
	{
		...
		{
			if (itemsInserted || doItemEasing)
				RepositionItems(keepItemPosBeforeNewItem);
			else
				PositionNewItems(keepItemPosBeforeNewItem);

			itemsInserted = false;
			keepItemPosBeforeNewItem = true;
			newItems.Clear();
		}
		...
		if (cachedPos != CachedTransform.position ||
			cachedRot != CachedTransform.rotation ||
			cachedScale != CachedTransform.lossyScale)
		{
			cachedPos = CachedTransform.position;
	...
	protected void CalcSnapItem()
	{
		...
		else
		{
			...
			scrollTime = Time.deltaTime * (scrollStopThresholdLog - Mathf.Log10(d)) / Mathf.Log10((d - d * scrollDecelCoef) / d);
