...
/// just like a button.
/// </remarks>
[AddComponentMenu("EZ GUI/Controls/List Button 3D")]
public class UIListButton3D : UIButton3D
{
	protected UIScroller list;
	public UIScroller List { get { return list; } }

	// Sets the scroll list that contains this list item.
	public void SetList(UIScroller c)
	{
		list = c;
	}
	...
	public override void OnInput(ref POINTER_INFO ptr)
	{
		...
		else if ((ptr.evt & whenToInvoke) == whenToInvoke)
			goto Invoke;

		return;

	Invoke:
		if ((ptr.evt & whenToInvoke) == whenToInvoke)
		{
	...
	new static public UIListButton3D Create(string name, Vector3 pos)
	{
		GameObject go = new GameObject(name);
		go.transform.position = pos;
		return (UIListButton3D)go.AddComponent(typeof(UIListButton3D));
	}
	...
	/// <returns>Returns a reference to the component.</returns>
	new static public UIListButton3D Create(string name, Vector3 pos, Quaternion rotation)
	{
		GameObject go = new GameObject(name);
		go.transform.position = pos;
