...
public abstract class AutoSpriteControlBase : AutoSpriteBase, IControl, IUIObject, IPackableControl
{
	...
	// Collider adjustment [12/17/2011 sonilics]
	// The offset of collider for make the collider can be different with the vertices in the control
	[HideInInspector]
	public float colliderLeftOffset;
	[HideInInspector]
	...
	public virtual string Text
	{
		...
		set
		{
			...
			if (spriteText == null)
			{
				...
				// new font system [12/17/2011 sonilics]
				if (FontManager.instance == null)
				{
					Debug.LogWarning("Warning: No FontManager exists in the scene. A FontManager with a default font is required to automatically add text to a control.");
					return;
				}
				else if (FontManager.instance.GetDefaultFont() == null)
				{
				...
				// Removed for new font system [1/29/2012 sonilics]
	...
	public virtual void Copy(SpriteRoot s, ControlCopyFlags flags)
	{
		...
		if ((flags & ControlCopyFlags.State) == ControlCopyFlags.State)
		{
			if (c is UIStateToggleBtn || !Application.isPlaying)
			{
				if (c.StateLabels != null)
				{
					StateLabels = new string[c.StateLabels.Length];
					if (StateLabels != null)
					{
						for (int i = 0; i < StateLabels.Length; ++i)
						{
							StateLabels[i] = c.StateLabels[i];
						}
					}
				}
			}
			...
			{
				if (StateLabels != null && c.StateLabels != null)
					for (int i = 0; i < StateLabels.Length && i < c.StateLabels.Length; ++i)
						StateLabels[i] = c.StateLabels[i];
			}
		}
	...
	[ContextMenu("Copy From other control")]
	public void CopyFromOtherControl()
	{
		foreach (AutoSpriteControlBase control in this.GetComponents(typeof(AutoSpriteControlBase)))
			if (control != this)
			{
				this.Copy(control);
				break;
	...
	public virtual void UpdateCollider()
	{
		...
		Vector3 min = m_spriteMesh.vertices[m_spriteMesh.vertexCountX * (m_spriteMesh.vertexCountY - 1)];
		Vector3 max = m_spriteMesh.vertices[m_spriteMesh.vertexCountX - 1];

		// For collider offset [1/29/2012 sonilics]
		switch (plane)
		{
			case SPRITE_PLANE.XY:
				min.x += colliderLeftOffset;
				min.y += colliderBottomOffset;
				max.x += colliderRightOffset;
				max.y += colliderTopOffset;
				break;

			case SPRITE_PLANE.XZ: break;
			case SPRITE_PLANE.YZ: break;
		}
	...
	public virtual string[] StateLabels
	{
		get { return null; }
		set { }
	}
