...
public struct POINTER_INFO
{
	...
	public enum INPUT_EVENT
	{
		...
		/// </remarks>
		NO_CHANGE = 0,								// Nothing happened from the last time we polled the device

		/// <remarks>
		/// The device has been pressed (mouse 
		/// button down, screen touched, etc)
		/// </remarks>
		PRESS = 1<<0,									// The device has been pressed (mouse button down, screen touched, etc)
		...
		/// </remarks>
		RELEASE = 1<<1,								// The device has been released but was not a tap (mouse button up, touch ended, etc)

		/// <remarks>
		/// The device has been released as a tap 
		/// (mouse button up, touch ended, etc)
		/// </remarks>
		TAP = (1<<2) | RELEASE,									// The device has been released as a tap (mouse button up, touch ended, etc)

		/// <remarks>
		/// The device was moved (mouse moved 
		/// with no buttons down)
		/// </remarks>
		MOVE = (1<<3),									// The device was moved (mouse moved with no buttons down)
		...
		/// </remarks>
		DRAG = (1 << 6)									// The device was moved while being pressed (mouse moved with button down, touch was dragged, etc)
	}
...
public struct KEYBOARD_INFO
{
	...
	public string textPlaceHolder; // Place holder support [sonilics]
#endif
...
public class UIManager : MonoBehaviour
{
	...
	*/
	...
	// Remove default font for new font system [12/17/2011 koduser]
	...
	// For resolution scale [5/6/2013 sonilics]
	/// <summary>
	/// The resolution scale between working mode and texture
	/// </summary>
	public float textureResolutionScale = 1.0f;

	// For screen scale [10/9/2013 sonilics]
	/// <summary>
	/// OrthographicSize Scale of UI render camera between working mode and running mode
	/// </summary>
	private float uiScreenScale = 1.0f;
	public float UIScreenScale
	{
		get { return uiScreenScale; }
		set { uiScreenScale = value; }
	}
...
// 				else
	...
	// Update layer mask [sonilics]
	public void SetCameraLayerMask(int index, LayerMask mask, float depth)
	{
		// Keep the index in a valid range:
		if (index >= uiCameras.Length)
			return;

		uiCameras[index].mask = mask;
		uiCameras[index].rayDepth = depth;
	...
	protected void DispatchInput()
	{
		...
		if (mouseTouchListeners != null)
		{
			for (int j = 0; j < numActivePointers; ++j)
				for (int i = 0; i < uiCameras.Length; ++i)
					// Disabled camera cannot raycast. [01/09/2014 sonilics]
#if UNITY_4_0 || UNITY_4_1 || UNITY_4_2 || UNITY_4_3 || UNITY_4_4 || UNITY_4_5 || UNITY_4_6 || UNITY_4_7 || UNITY_4_8 || UNITY_4_9
					if (uiCameras[i].camera.gameObject.activeInHierarchy && uiCameras[i].camera.enabled)
#else
					if (uiCameras[i].camera.gameObject.active  && uiCameras[i].camera.enabled)
#endif
					{
						// Temporary resolve a bug which caused the camera in pointers missing after switch scene. [10/26/2011 sonilics]
						if (pointers[i, activePointers[j]].camera != uiCameras[i].camera)
							pointers[i, activePointers[j]].camera = uiCameras[i].camera;
						...
						{
							// Notice other camera pointer release event [6/25/2013 sonilics]
							if (pointers[i, activePointers[j]].evt == POINTER_INFO.INPUT_EVENT.RELEASE)
							{
								// Notify the targetObj of the event:
								tempPtr.Copy(pointers[i, activePointers[j]]);
								...
								for (int k = 0; k < uiCameras.Length; ++k)
								{
									...
									if (pointers[k, activePointers[j]].targetObj != null)
									{
										pointers[k, activePointers[j]].targetObj.OnInput(tempPtr);
									}
								}
							}
						...
						curPtr.evt == POINTER_INFO.INPUT_EVENT.TAP)
					{
						...
						if (tempObj != curPtr.targetObj)
						{
							...
							if (tempObj != null)
							{
								...
								if (curPtr.evt != POINTER_INFO.INPUT_EVENT.TAP && curPtr.evt != POINTER_INFO.INPUT_EVENT.RELEASE && !blockInput)
	...
	protected void PollKeyboard()
	{
		...
		if (!Application.isEditor)
		{
			...
			// Fix IME bug, iKeyboard.active may be false in IME mode.[10/8/2013 sonilics]
