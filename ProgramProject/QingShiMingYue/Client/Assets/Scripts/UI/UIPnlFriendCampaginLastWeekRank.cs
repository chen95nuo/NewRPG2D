...
public class UIPnlFriendCampaginLastWeekRank : UIModule
{
	...
	public SpriteText lastWeekRankLabel;
	...
	private IEnumerator FillRankList(List<com.kodgames.corgi.protocol.FCRankInfo> rankInfos)
	{
		...
		if (rankList.Count <= 0)
			notList.Text = GameUtility.GetUIString("UIPnlFriendCampaginLastWeekRank_NotList");
		else
	...
	{
		//发送查询详细协议，参数为周期约定
		RequestMgr.Inst.Request(new QueryFCPointDetailReq(_FCRankType.LastPeriod));
	}

	private void QueryRankList()
	{
		this.waitRequest = true;
		RequestMgr.Inst.Request(new QueryFCRankReq(_FCRankType.LastPeriod));
	}
