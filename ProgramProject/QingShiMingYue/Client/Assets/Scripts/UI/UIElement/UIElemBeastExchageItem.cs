...
public class UIElemBeastExchageItem : MonoBehaviour
{
	...
	private bool IsMeetCondition(object item)
	{
		if (ItemInfoUtility.ExchangeIsOptionCost(item))
			return selectedGuids.ContainsKey(item) && selectedGuids[item] != null;
		else
	...
	public bool IsCostRousoucesIDEquals(List<KodGames.ClientClass.Cost> costRous)
	{
		for (int i = 0; i < costRous.Count; i++)
			if (i > 0 && costRous[i].Id == costRous[0].Id)
				return true;
		return false;
	}
	...
	private void ShowCostView(object cost)
	{
		...
		if (ItemInfoUtility.ExchangeIsOptionCost(cost))
			SysUIEnv.Instance.ShowUIModule<UIPnlChooseCard>(cost, new UIPnlChooseCard.OnChooseCardSuccessDel(OnResetSelectGuid));
		else  //显示奖励物品具体信息
	...
	{
		OnClickFindEastSeaBtn();
		if (costs == null || costs.Count < GetIconCount())
		{
			SysUIEnv.Instance.ModulePool.ShowModule(typeof(UIPnlTipFlow), GameUtility.GetUIString("UIPnlEastSeaFindFairyMain_PleaseContext"));
			return;
		}
		if (IsCostRousoucesIDEquals(costs))
		{
			SysUIEnv.Instance.ModulePool.ShowModule(typeof(UIPnlTipFlow), GameUtility.GetUIString("UIPnlEastSeaFindFairyMain_PleaseNoEquitItem"));
			return;
		}
		RequestMgr.Inst.Request(new BeastExchangeShopReq(zentiaExchange.ExchangeId, zentiaExchange.Index, costs, OnExchangeItemSuccess));

	}


	//兑换仙缘按钮回调
	public bool OnExchangeItemSuccess(KodGames.ClientClass.Reward reward)
	{
		//处理更新
		exchangeBtn.SetControlState(UIButton.CONTROL_STATE.DISABLED);
		zentiaExchange.IsAlreadyExchanged = true;
		ClearData(true);
		SetData(zentiaExchange);

		SysUIEnv.Instance.ModulePool.ShowModule(typeof(UIPnlTipFlow), SysLocalDataBase.GetRewardDesc(reward, true, false));
		return true;
	}
