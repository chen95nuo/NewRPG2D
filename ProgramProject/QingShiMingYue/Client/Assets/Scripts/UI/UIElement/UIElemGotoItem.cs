...
public class UIElemGotoItem : MonoBehaviour
{
	...
	public void SetData(GetWay getway)
	{
		...
		{
			getWayTipLabel.Text = GameUtility.FormatUIString("UIDlgItemGetWay_GetWay_Label_NoDungeon", GameDefines.textColorBtnYellow.ToString(), getway.desc);
			getWayDesc.Text = "";
		}
	...
	private void HideGetWayUI(bool tf)
	{
		getWayDesc.Hide(tf);
		gotoBtn.Hide(tf);
		getWayTipLabel.Hide(!tf);
	}
}
