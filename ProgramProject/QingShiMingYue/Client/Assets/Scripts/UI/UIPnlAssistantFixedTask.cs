...
public class UIPnlAssistantFixedTask : UIPnlAssistantTaskBase
{
	public override bool OnShow(_UILayer layer, params object[] userDatas)
	{
		...
		SysUIEnv.Instance.GetUIModule<UIPnlAssistant>().ChangeTab(_UIType.UIPnlAssistantFixedTask);
	...
	IEnumerator FillList()
	{
		...
		if (SysLocalDataBase.Inst.LocalPlayer.QuestData.Quests == null)
			SysUIEnv.Instance.ModulePool.ShowModule(typeof(UIPnlTipFlow), GameUtility.GetUIString("UIPnlAssistant_FixedLevel_tips"));
		else
		{
			//由本地数据获取固定任务列表临时存储一份
			...
			{
				if ((ConfigDatabase.DefaultCfg.QuestConfig.GetQuestById(quest.QuestId).type != QuestConfig._Type.RepeatedQuest) ||
					(
		...
		// Set Empty Label.
		if (SysLocalDataBase.Inst.LocalPlayer.LevelAttrib.Level < ConfigDatabase.DefaultCfg.LevelConfig.GetPlayerLevelByOpenFunciton(_OpenFunctionType.DailyQuestReward))
			emptyLabel.Text = GameUtility.GetUIString("UIPnlAssistant_FixedLabel");
		else
			if (taskList.Count <= 0)
				emptyLabel.Text = GameUtility.GetUIString("UIPnlAssistant_FixedLabel_Not");
