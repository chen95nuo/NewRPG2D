...
public class UICtlInspector : Editor, IGUIHelper
{
	...
	{
		if (control is SpriteRoot)
		{
			BeginMonitorChanges();
			ShowSpriteRootTemplateSetting();
			ShowTextureStyleSettings();
			EndMonitorChanges();
		}
	...
	void ShowSpriteRootTemplateSetting()
	{
		SpriteRoot sprite = control as SpriteRoot;
		...
		List<string> templateNames = UITemplateFormatter.GetTemplateNameList(typeof(SpriteRoot), true);
		int selectIndex = templateNames.FindIndex(
			delegate(string name)
			{
				return name.Equals(sprite.templateName);
			});
		selectIndex = EditorGUILayout.Popup("Name", selectIndex, templateNames.ToArray());
		sprite.templateName = selectIndex > 0 && selectIndex < templateNames.Count ? templateNames[selectIndex] : "";
	...
	void ShowTextureStyleSettings()
	{
		...
		EditorGUILayout.BeginVertical("Toolbar");
		EditorGUILayout.LabelField("Texture Style :");
		
		sprite.leftBorder = EditorGUILayout.IntField("Left Border", sprite.leftBorder);
		sprite.rightBorder = EditorGUILayout.IntField("Right Border", sprite.rightBorder);
		sprite.topBorder = EditorGUILayout.IntField("Top Border", sprite.topBorder);
		sprite.bottomBorder = EditorGUILayout.IntField("Bottom Border", sprite.bottomBorder);
		sprite.xTextureTile = EditorGUILayout.Toggle("Tile Horizontal", sprite.xTextureTile);
		sprite.yTextureTile = EditorGUILayout.Toggle("Tile Vertical", sprite.yTextureTile);

		// xMirror 
		EditorGUI.BeginDisabledGroup(sprite.xTextureTile);
		sprite.xMirror = EditorGUILayout.Toggle("Mirror Horizontal", sprite.xMirror);
		EditorGUI.EndDisabledGroup();
		// yMirror 
		EditorGUI.BeginDisabledGroup(sprite.yTextureTile);
		sprite.yMirror = EditorGUILayout.Toggle("Mirror Vertical", sprite.yMirror);
		EditorGUI.EndDisabledGroup();

		sprite.keepBorderAspectRate = EditorGUILayout.Toggle("Constrain Proportions", sprite.keepBorderAspectRate);
		sprite.piercedStyle = EditorGUILayout.Toggle("Pierced", sprite.piercedStyle);		

		EditorGUILayout.EndVertical();

		GUILayout.Space(190.0f);
		EditorGUILayout.EndVertical();
	}

	void ShowColliderOffsetSettings()
	{
		AutoSpriteControlBase sprite = control as AutoSpriteControlBase;

		EditorGUILayout.BeginVertical();
		
		EditorGUILayout.BeginVertical("Toolbar");
		EditorGUILayout.LabelField("Collider Offset :");
		sprite.colliderLeftOffset = EditorGUILayout.FloatField("Left", sprite.colliderLeftOffset);
		sprite.colliderRightOffset = EditorGUILayout.FloatField("Right", sprite.colliderRightOffset);
		sprite.colliderTopOffset = EditorGUILayout.FloatField("Top", sprite.colliderTopOffset);
		sprite.colliderBottomOffset = EditorGUILayout.FloatField("Bottom", sprite.colliderBottomOffset);
		EditorGUILayout.EndVertical();
