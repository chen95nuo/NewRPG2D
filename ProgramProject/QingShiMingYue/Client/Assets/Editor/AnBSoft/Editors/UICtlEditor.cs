...
public class UICtlEditor : EditorWindow, IGUIHelper, IGUIScriptSelector
{
	...
	// Anchor
	protected SpriteRoot.ANCHOR_METHOD lastAnchor = 0;
	protected SpriteRoot.ANCHOR_METHOD anchor = 0;
	...
	public void OnSelectionChange()
	{
		...
		if (somethingChanged)
		{
			if (Selection.activeGameObject != null)
			{
				...
				SpriteRoot spriteRoot = (SpriteRoot)Selection.activeGameObject.GetComponent("SpriteRoot");
				if (spriteRoot != null)
				{
					lastAnchor = spriteRoot.anchor;
					anchor = spriteRoot.anchor;
				}
			}
	...
	public void OnGUI()
	{
		...
		// SpriteRoot for anchor edit.
		SpriteRoot spriteRoot = null;
		if (Selection.activeGameObject != null)
			spriteRoot = (SpriteRoot)Selection.activeGameObject.GetComponent("SpriteRoot");
	...
	// EZ1.08. Get All GameObjects in Scene.
...
*/
	...
	// Anchor edit
	public void ShowAnchorSetting(SpriteRoot spriteRoot)
	{
		anchor = (SpriteRoot.ANCHOR_METHOD)EditorGUILayout.EnumPopup("Anchor", anchor);

		if (anchor != lastAnchor)
		{
			SetAchorPositon(spriteRoot);
		}
	}

	public void SetAchorPositon(SpriteRoot spriteRoot)
	{
		// Set the SpriteRoot 's anchor.
		spriteRoot.anchor = anchor;

		Vector3 lastAnchorPos = GetOffset(lastAnchor, spriteRoot.width, spriteRoot.height);
		Vector3 anchorPos = GetOffset(anchor, spriteRoot.width, spriteRoot.height);

		// Set the all child Positon in spriteRoot.
		RePositionGameObject(spriteRoot.gameObject, (anchorPos - lastAnchorPos));

		// Change the lastAnchor data.
		lastAnchor = anchor;
	}

	private void RePositionGameObject(GameObject obj, Vector3 offset)
	{
		if (obj == null)
			return;
		...
		{
			obj.transform.GetChild(index).transform.localPosition += new Vector3(-offset.x, -offset.y, 0f);
		}
	}

	private Vector3 GetOffset(SpriteRoot.ANCHOR_METHOD spriteAnchor, float width, float height)
	{
