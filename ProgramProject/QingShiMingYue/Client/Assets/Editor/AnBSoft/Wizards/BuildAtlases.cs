...
{
	public string atlasFolder = "WorkAssets/Textures/UIAtlas";	// Folder in which to place generated atlases
	public int maxAtlasSize = 1024;		// Maximum pixel dimensions of a generated atlas
	public int padding = 0;				// Pixels of padding to place around each packed texture
	public bool trimImages = false;		// Whether or not to trim blank pixels from sprite frames automatically.
	...
	public bool onlyScanProjectFolder = false;	// Scan the entire "Assets" folder for prefabs containing packable sprites.
#else
	...
	public bool haltOnTextureResized = false;

	// [Modified by kinger]
	// To add outline to image.
	public bool addOutline = true; // Whether or not to add outline to image.
	...
	void LoadSettings()
	{
		...
		haltOnTextureResized = 1 == PlayerPrefs.GetInt("BuildAtlases.haltOnTextureResized", haltOnNullMaterial ? 1 : 0);

		// [Modified by kinger]
		// To add outline to image.
		addOutline = 1 == PlayerPrefs.GetInt("BuildAtlases.addOutline", addOutline ? 1 : 0);
	}
	...
	void SaveSettings()
	{
		...
		PlayerPrefs.SetInt("BuildAtlases.haltOnTextureResized", haltOnTextureResized ? 1 : 0);

		// [Modified by kinger]
		// To add outline to image.
		PlayerPrefs.SetInt("BuildAtlases.addOutline", addOutline ? 1 : 0);
		PlayerPrefs.Save();
	}


	[UnityEditor.MenuItem("Tools/A&B Software/Build Atlases &a")]
	// Return BuildAtlases for building tool [1/29/2012 sonilics]
	public static BuildAtlases BuildSpriteAtlases()
	{
	...
	// Make public for building tool [1/29/2012 sonilics]
...
// 		atlas = (Texture2D) AssetDatabase.LoadAssetAtPath(projectRelativePath, typeof(Texture2D));
		...
		try
		{
			    ...
			    texList.uvs[0].height * atlas.height != (texList.trimmedTextures[0]).height)
			{
				...
				if (haltOnTextureResized)
					return;
			}
		}
		catch (System.Exception e)
		{
			Debug.LogError("Try to access destroyed texture : " + texList.material.ToString () + " " + texList.texPath);
			EditorUtility.DisplayDialog("Exception occurs", "Try to access destroyed texture : " + texList.material.ToString () + " " + texList.texPath, "Ok");
			throw e;
		}
		...
		if (addOutline)
		{
			...
			for (int j = 0; j < texList.uvs.Length; ++j)
			{
				...
				texList.uvs[j].y += dy;
				texList.uvs[j].width -= dx * 2;
				texList.uvs[j].height -= dy * 2;
			}
		}
	...
	void ProcessFrame(Texture2D tex, bool dontTrim, ref Texture2D newTex, ref SPRITE_FRAME frameInfo)
	{
		...
		if (addOutline)
		{
			Texture2D otlTex = new Texture2D(tex.width + 2, tex.height + 2, TextureFormat.ARGB32, false);

			otlTex.SetPixels(1, 1, tex.width, tex.height, tex.GetPixels(), 0);
			otlTex.SetPixel(0, 0, tex.GetPixel(0, 0));
			otlTex.SetPixel(0, otlTex.height - 1, tex.GetPixel(0, tex.height - 1));
			otlTex.SetPixel(otlTex.width - 1, 0, tex.GetPixel(tex.width - 1, 0));
			otlTex.SetPixel(otlTex.width - 1, otlTex.height - 1, tex.GetPixel(tex.width - 1, tex.height - 1));

			// Top & Bottom line.
			for (int c = 0; c < tex.width; c++)
			{
				otlTex.SetPixel(c + 1, 0, tex.GetPixel(c, 0));
				otlTex.SetPixel(c + 1, tex.height + 1, tex.GetPixel(c, tex.height - 1));
			}

			// Left & Right line.
			for (int c = 0; c < tex.height; c++)
			{
				otlTex.SetPixel(0, c + 1, tex.GetPixel(0, c));
				otlTex.SetPixel(tex.width + 1, c + 1, tex.GetPixel(tex.width - 1, c));
			}
		...
		if (addOutline)
			frameInfo.textureSize = new Vector2(newTex.width - 2, newTex.height - 2);
		else
			frameInfo.textureSize = new Vector2(newTex.width, newTex.height);
	}
