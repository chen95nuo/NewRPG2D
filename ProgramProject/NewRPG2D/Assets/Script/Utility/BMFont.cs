...
public class BMFont
{
    ...
    static void Font()
    {
        Material mtr = Resources.Load<Material>("Font/Number/NumberMaterial_1"); //把我们创建的材质球加载进来
        Texture2D texture = Resources.Load<Texture2D>("Font/Number/NumberTexture_1"); //把我们在位图制作工具生成的图片加载进来
        mtr.mainTexture = texture;
        Font font = Resources.Load<Font>("Font/Number/NumberFont_1"); //把我们创建的字体加载进来
        XmlDocument xml = new XmlDocument();
        xml.Load(Application.dataPath + "/Resources/Font/Number/Number_1.fnt");//这是在BMFont里得到的那个.fnt文件,因为是xml文件，所以我们就用xml来解析
